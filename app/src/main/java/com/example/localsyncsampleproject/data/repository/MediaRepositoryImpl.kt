...
class MediaRepositoryImpl @Inject constructor(
    ...
    override suspend fun isPersonCounts(): Flow<Int> = database.mediaDao().getIsPersonPhotos()
    ...
    override fun createMediaData(): Int { // 갤러리 Media 개수만 카운팅해서 반환
    ...
    override suspend fun syncMediaData(): List<Media> {
            ...
            try {
                query?.use { cursor ->
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        val dataStoreLastTime = syncTimeRepository.observeLastTime().first() // 마지막 업로드 시간
                        ...
                        val hashMap: HashMap<String, Double?> = calculateGPSInfo(sLat, sLatR, sLon, sLonR)
        ...
        return mediaList // 확인을 위해서 return 해줌
    }
    ...
    // 지역 정보 카운팅
    override fun exifGPSInfoCounts(): Flow<Int> {
        return flow {
            var counts: Int = 0
            val columns = arrayOf(
                MediaStore.Images.Media.DATA,           // Disk 미디어 절대 파일 시스템 경로
            ...
            try {
                query?.use { cursor ->
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        try {
                            // lat, lng를 Exif에서 파싱해서 넣어주기
                            if (Utils.getExifData(context, contentUri)) {
                                counts += 1
                            }
                        } catch (e: Exception) {
                            Log.e("error => ", "$e")
